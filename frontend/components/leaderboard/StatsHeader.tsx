    ...
    <div className="flex flex-col bg-background-dark z-10">
      <div className="p-6 pb-2">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <h2 className="text-3xl font-bold tracking-tight text-white">Leaderboard</h2>
        ...
        <div className="flex flex-col xl:flex-row gap-6 justify-between items-end border-b border-[#1A1A1A] pb-0">
          ...
          <div className="flex items-center divide-x divide-[#1A1A1A] pb-3 w-full xl:w-auto overflow-x-auto">
            <div className="px-4 first:pl-0">
              <p className="text-[11px] uppercase tracking-wider text-[#A1A1AA] font-semibold">Total Bots</p>
              <p className="text-lg font-bold font-mono text-white">{stats.totalBots.toLocaleString()}</p>
            </div>
            <div className="px-4">
              <p className="text-[11px] uppercase tracking-wider text-[#A1A1AA] font-semibold">24H Volume</p>
              <p className="text-lg font-bold font-mono text-white">{formatUsd(stats.volume24hUsd)}</p>
            </div>
            <div className="px-4">
              <p className="text-[11px] uppercase tracking-wider text-[#A1A1AA] font-semibold">Avg 30D</p>
              <p className="text-lg font-bold font-mono text-[#22c55e]">{formatPct(stats.avg30dReturnPct)}</p>
            </div>
            <div className="px-4 last:pr-0">
              <p className="text-[11px] uppercase tracking-wider text-[#A1A1AA] font-semibold">Top Performer</p>
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-full bg-gradient-to-br from-blue-500 to-purple-600"></div>
                <p className="text-sm font-bold truncate max-w-[150px] text-white">
